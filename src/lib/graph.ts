...
export class DGraph {
	...
	public addNode(name:string, path:string, data?: Object):Node {
		...
		if(this.hasNode(name)) { 
			throw new Error(`possible duplicate pkg \`${name}\` @ ${path} & ${this.getNode(name).path}`)
		}

		let newNode = new Node(name, path, data);
		console.log("adding", newNode.name);
	...
	public getNode(n:Node|string):Node {
		...
		if(node === undefined) {
			throw new Error(`missing node named ${this._name(n)}`)
		}
	...
	addEdge(from:Node | string, to:Node | string):boolean {
		...
		if(fromNode === toNode) throw new Error(`cannot add loopback edge at ${toNode.name}`);
		console.log("Add edge ", fromNode.name, "->", toNode.name);

		let dst = this._inEdges.get(toNode);
		let org = this._outEdges.get(fromNode)

		if(!org || !dst) throw new Error("missing nodes");
	...
	removeEdge(from:Node | string, to:Node | string) {
		...
		console.log("Remove edge ", fromNode.name, "->", toNode.name);
	...
	dependenciesOf(node:Node | string , leavesOnly=false) {

	}
	dependantsOf(node:Node | string, leavesOnly=false) {
	 ...
	 */
	overallOrder(leavesOnly=false):Array<Node>[] {
		let seen = new Set();
		let Q = new Queue();
		let nCnt = 0;
		...
		if(Q.isEmpty()) throw new CycleError({reason: "invalid DAG!"});
...
class CycleError extends Error {
    ...
    constructor(meta = {}) {
        ...
        Error.captureStackTrace(this, CycleError);

        let proto = Object.getPrototypeOf(this);
        proto.name = 'CycleError';
